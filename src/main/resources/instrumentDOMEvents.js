...
/**
 * Instrumentation of the JavaScript language in order to better understand the
 * occurrence of DOM events. Specifically, DOM events which are generated by
 * user-actions are tied to their callback functions. The ordering of event
 * listeners is captured by the instrumentation process as well. For a given
 * webpage, this instrumentation must be done after all DOM elements have been
 * loaded but before any event listeners are added by the original
 * web-application.
 */
...
function exitSnapshot(info) {
	...
	trace += 'send(JSON.stringify({messageType: "FUNCTION_EXIT", timeStamp: Date.now(), targetFunction: "'
			...
			+ '",lineNo: '
			+ info.line
			+ ', scopeName: "n/a", counter:traceCounter++}));';
	return trace;
}
...
function EventListenerWrapper(targetPrototype, listenerSetter, listenerRemover) {
	if ((targetPrototype instanceof Object)
			&& (listenerSetter.linsteners instanceof Object)
			&& typeof (this.restore) === "function") {
		this.restore.apply(this, arguments);
	} else if ((targetPrototype instanceof Object)
			&& typeof (listenerSetter) === "string"
			&& typeof (listenerRemover) === "string") {
		var copyOfSetterFunction = targetPrototype[listenerSetter];
		targetPrototype.originalSetter = listenerSetter;
		if (Object.prototype.toString.apply(targetPrototype[listenerSetter]) === '[object Function]'
				&& targetPrototype[listenerSetter].name !== "_clematisSetter") {
			...
			function _clematisSetter(arg0, arg1) {
				...
				if (Object.prototype.toString.apply(arg0) === '[object Function]') {
					...
					// overwritten event listener is the wrapper function,
					// contains orig
					overwrittenListenerAdder = arg0 = overwriteEventListener(
							oldListenerAdder, this, arg1);
					...
					// overwritten event listener is the wrapper function,
					// contains orig
					overwrittenListenerAdder = arg1 = overwriteEventListener(
							oldListenerAdder, this, arg0);
				}

				// Calling the seter (e.g. addEventListener, setAttribute) with
				// this
				// and the original arguments. For example, for
				// "setTimeout('incrementCounter', 1000)"
				// as a handler, seter = setTimeout, arguments =
				// ['overwrittenEventListener', 1000] where
				// 'overwrittenEventListener' is a wrapper containing the line
				// incrementCounter.apply(...)
			...
			targetPrototype[listenerSetter] = _clematisSetter; // overwrite the
																// original
																// function that
																// adds
																// event-listeners
	...
	function overwriteEventListener(originalEventListener, object1, eventType) {
		return function overwrittenEventListener() {
			...
			case "addEventListener":
				dom_logger.logDOMEvent(eventType, this, originalEventListener);
				break;
			default:
 ...
 * 
 * Overwrites functions that register eventlisteners for a specific type of
 * object
 */
function replaceListenerAdder(targetObjectPrototype, objectName, o) {
	if (alreadyOverwritten.hasOwnProperty(objectName)) { // Prototype (e.g.
															// Window, Button)
															// already added
		return objectName;
	}

	if (targetObjectPrototype.hasOwnProperty("addEventListener")
			|| targetObjectPrototype.addEventListener instanceof Function) {
		// Replace/wrap addEventListener method
		alreadyOverwritten[objectName] = new EventListenerWrapper(
				targetObjectPrototype, "addEventListener",
				"removeEventListener");
	}
	return objectName;
};
 ...
 * getLevel1Events
 * 
 * Get all properties for prototypes (that are used in the webpage). Then
 * iterates through all these properties and notes those beginning with "on"
 * (e.g. "onclick", "onhover", etc.). A list of these are saved to the
 * knownValidLevel1Events[] array with index 'name', the 'name' of the
 * prototype.
 * 
 */
function getLevel1Events(object) {
	...
	if (!objectName || objectName === "Object" || objectName.length === 0) {
		...
		if (objectName.length === 0) {
			objectName = Object.getPrototypeOf(object).constructor.toString()
	...
	if (knownValidLevel1Events.hasOwnProperty(objectName)) {
		// Ignores duplicates with same name (e.g. multiple
	...
	for (k = 0; k < objectProperties.length; k++) {
		...
		if (currentProp.indexOf("on") === 0
				&& propertiesToBeReturned.indexOf(currentProp) === -1) {
			propertiesToBeReturned.push(currentProp);
		}
	}

	// Save to array incase this method called again on already processed
...
function overrideIndividualDOMLevel1(object) {
	...
	for (j = 0; j < validEvents.length; j++) {
		...
		Object.defineProperty(object, singleEvent,
				_clematisReplacementDescriptors[singleEvent]);

		if (defaultLevel1Event
				&& (Object.prototype.toString.call(defaultLevel1Event) === '[object Function]')
				&& (defaultLevel1Event.name === singleEvent)) {
			// TODO: Not surefire way to extract actual function
			/*
			 * [1]: function name [2]: arguments [3]: function body
			 */
			var astRoot = esprima.parse(defaultLevel1Event, {
				range : true,
				loc : true
			});
			...
			newArguments = defaultLevel1Event.toString().substring(
					defaultLevel1Event.toString().indexOf('(') + 1,
					defaultLevel1Event.toString().indexOf(')'));
			newArguments = newArguments.split(',');

			// Call Expressions
			actualFunction = esmorph.modify(defaultLevel1Event.toString(),
					esmorph.Tracer.CallExpression(callSnapshot));

			// Return Statements
			actualFunction = esmorph.modify(actualFunction, esmorph.Tracer
					.ReturnStatement(returnSnapshot));

			// Function Declarations
			actualFunction = esmorph.modify(actualFunction, esmorph.Tracer
					.FunctionEntrance(enterSnapshot));
			actualFunction = esmorph.modify(actualFunction, esmorph.Tracer
					.FunctionExit(exitSnapshot));
			newBody = actualFunction.toString().substring(
					actualFunction.toString().indexOf('{') + 1,
					actualFunction.toString().lastIndexOf('}'));
			newFn = new Function("return function " + defaultLevel1Event.name
					+ "(" + newArguments.join(',') + "){" + newBody + "}")();
...
/*
 * When webpage has loaded replace all listeners. Iterates through all document
 * elements, and overwrites their original event handlers with our wrapper.
 */
document.addEventListener("DOMContentLoaded",
		function replaceDOMLevel1ForAll() {
			var i, allElements = document.querySelectorAll("*");

			// Removes this DOM event which was added below once all listeners
			// have been replaced.
			this.removeEventListener("DOMContentLoaded",
					replaceDOMLevel1ForAll, false);
...
// Call the overwrite function on the actual document, before wrapping its
// elements
...
if (_detectedBrowser === 'chrome') {
	...
	replaceListenerAdder(HTMLTableCaptionElement.prototype,
			"HTMLTableCaptionElement");
	replaceListenerAdder(HTMLTableColElement.prototype, "HTMLTableColElement");
	replaceListenerAdder(HTMLTableSectionElement.prototype,
	...
	// replaceListenerAdder(HTMLBaseFontElement.prototype,
	// "HTMLBaseFontElement");
}